...
for gen = 1:max_gen
    ...
    if gen > 50
        ...
        if std_recent < 1e-4 && mean(delta(end-10:end)) < 5e-4 && avg_gap < 1e-3
...
K = 200;              % 检查最近 K 代
eps_tol = 0.005;      % 容差
[is_conv, conv_val, ratio] = checkConvergence(best_fitness_history, K, eps_tol);

fprintf('\n=== 收敛性分析结果（checkConvergence） ===\n');
fprintf('参考收敛值：%.6f\n', conv_val);
fprintf('容差范围：±%.4f\n', eps_tol);
fprintf('稳定在该范围内的代数比例：%.2f%%\n', ratio * 100);
...
for j = 1:N
    ...
    if rand() < 0.3
        ...
        j1 = find(mutated.x_ij(i1, :) == 1);
        j2 = find(mutated.x_ij(i2, :) == 1);
        if length(j1) == 1 && length(j2) == 1 && j1 ~= j2
            mutated.x_ij(i1, :) = 0;
            mutated.x_ij(i2, :) = 0;
            mutated.x_ij(i1, j2) = 1;
            mutated.x_ij(i2, j1) = 1;
        end
    end
end