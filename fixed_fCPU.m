...
while iter < max_iter  
    ...
    cvx_begin
        ...
        subject to
            ...
            % 时延约束，固定CPU频率后时延约束变为线性
            for j = 1:N
                T0 +  s * T0 * n_flop * f_s(j)/f_CPU  + ( s * T0 * f_s(j) ) / R <= T_max;
            ...
            for j = 1:N
                ...
                P_static + C_S * f_s(j) + C_CPU * pow_pos(f_CPU/100, 3) + P_tx(j) <= alpha_f(j) * sum(w(:, j));
            end
...
disp('--- 时延约束检查 ---');
for j = 1:N
    delay = T0 + f_s(j)/f_CPU * s * T0 * n_flop + (f_s(j) * s * T0)/R;
...
for j = 1:N
    ...
    power_lhs = P_static + C_S * f_s(j) + C_CPU * (f_CPU/100)^3 + P_tx(j);
    power_rhs = sum(full(w(:, j)) .* alpha_f(j));
